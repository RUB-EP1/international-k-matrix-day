...
md"""
$\hat{K}_{ij}(s) = \left(\sum \frac{g_i^\alpha g_j^\alpha}{m_\alpha^2-s} + f_{ij} \frac{m_0^2-s_0^\text{scatt}}{s-s_0^\text{scatt}}\right) f_{A0}(s)$
"""
...
begin
	...
	function amplitude(K::Kmatrix, m)		
		...
		non_pole_part = (m0sq-s0_scatt) / (m^2-s0_scatt) * K.nonpoles
...
model = let
	...
	MG = [
		...
		(M=0.55817, gs = [0.00000, 0.00000, 0.55639, 0.85679, -0.79658]),
